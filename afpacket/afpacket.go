...
func (h *TPacket) setUpRing() (err error) {
	...
	case TPacketVersion1, TPacketVersion2:
		var tp tpacket_req
		tp.tp_block_size = uint32(h.opts.blockSize)
		tp.tp_block_nr = uint32(h.opts.numBlocks)
		tp.tp_frame_size = uint32(h.opts.frameSize)
		tp.tp_frame_nr = uint32(h.opts.framesPerBlock * h.opts.numBlocks)
